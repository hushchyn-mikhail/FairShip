...
def execute(TaggerHits, debug=0):
    ...
    # PatRec in zx plane
    tracks_zx = pat_rec_plane(TaggerHits_x, coord='x', min_hits=2, max_shared_hits=0)
    
    # PatRec in zy plane
    tracks_zy = pat_rec_plane(TaggerHits_y, coord='y', min_hits=2, max_shared_hits=0)
    ...
    for i_track in range(len(track_combinations)):
        ...
        if len(atrack['hits_x']) >= 3 and len(atrack['hits_y']) >= 3:
            track_hits[i_track] = {}
            track_hits[i_track]['hits_x'] = atrack['hits_x']
            track_hits[i_track]['hits_y'] = atrack['hits_y']
    ...
    for ahit1 in TaggerHits:
        for ahit2 in TaggerHits:
            ...
            for ahit3 in TaggerHits:
                ...
                in_bin = hit_in_window(z3, x3, k_bin, b_bin, window_width=2.0)#3
                if in_bin:
...
def reduce_clones(long_tracks, coord='x', min_hits=3, max_shared_hits=2):
    ...
    for i_track in np.argsort(n_hits)[::-1]:
        ...
        new_track = {}
        new_track['hits_'+coord] = []
        new_track[coord+'_'+coord] = []
        new_track['z_'+coord] = []
        n_shared_hits = 0

        for i_hit in range(len(atrack['hits_'+coord])):
            ahit = atrack['hits_'+coord][i_hit]
            new_track['hits_'+coord].append(ahit)
            new_track[coord+'_'+coord].append(atrack[coord+'_'+coord][i_hit])
            new_track['z_'+coord].append(atrack['z_'+coord][i_hit])
            if ahit['digiHit'] in used_hits: #digiHit
                n_shared_hits += 1

        if len(new_track['hits_'+coord]) >= min_hits and n_shared_hits <= max_shared_hits:
            short_tracks.append(new_track)
            for ahit in new_track['hits_'+coord]:
